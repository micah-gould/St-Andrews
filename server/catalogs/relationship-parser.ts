...
export function parseRelationshipText(text: string): RelationshipExpression | null {
...
export function extractModuleCodes(expression: RelationshipExpression | null | undefined, seen = new Set<string>()) {
  if (!expression) return seen;
  if (expression.type === 'module') {
    if (expression.code) seen.add(expression.code);
    return seen;
  }
...
export function evaluateBlocked(expression: RelationshipExpression | null | undefined, excluded: Set<string>) {
...
export function describeExpression(expression: RelationshipExpression | null | undefined) {
  if (!expression) return 'None';
  if (expression.type === 'module') return expression.code || 'None';
...
function parseAndGroup(segment: string): RelationshipExpression | null {
...
function enclosesWholeExpression(text: string) {
  let depth = 0;
