...
export function parseRelationshipText(
  ...
  const groups = splitTopLevel(body, " OR ")
    .map((segment) => parseAndGroup(segment))
    .filter(Boolean);
...
export function extractModuleCodes(
...
export function evaluateBlocked(
  ...
  if (expression.type === "and")
    return (expression.children || []).some((child) =>
      evaluateBlocked(child, excluded),
    );
