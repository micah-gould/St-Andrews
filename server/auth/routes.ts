...
import type { AuthedRequest, PartialAuthedRequest } from '../../src/types/server.types';
...
export async function attachUser(req: PartialAuthedRequest, _res: Response, next: NextFunction) {
...
export function requireAuth(req: PartialAuthedRequest, res: Response, next: NextFunction) {
...
router.post('/signup', authLimiter, async (req, res) => {
  ...
  try {
    ...
    const { token, maxAgeMs } = signSessionToken(user, { remember: !!remember });
...
router.post('/login', authLimiter, (req, res, next) => {
  passport.authenticate('local', { session: false }, (err, user, info) => {
    ...
    const { token, maxAgeMs } = signSessionToken(user, { remember: !!req.body?.remember });
    setAuthCookie(res, token, maxAgeMs);
...
router.post('/reset-password', authLimiter, async (req, res) => {
  ...
  try {
    ...
    const { token: jwtToken, maxAgeMs } = signSessionToken(record.user, { remember: false });
