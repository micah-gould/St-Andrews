...
async function fetchYearRows(school, year) {
  ...
  const html = await fetch(`${SEARCH_BASE}&${params.toString()}`).then(
...
async function main() {
  ...
  const files = (await fs.readdir(catalogDir)).filter((file) =>
    file.endsWith(".json"),
  );
  const missing = [];
  ...
  console.log(
    `[audit] found ${missing.length} modules with no extracted requirement text`,
  );

  const offeringIndex = await buildOfferingIndex();
  console.log(
    `[audit] built offering index for ${offeringIndex.size} module codes`,
  );
  const findings = [];
  let checked = 0;
  ...
  for (const item of missing) {
    ...
    for (const offer of offers) {
      const html = await fetch(offer.url)
      ...
      console.log(
        `[audit] found live requirement headings for ${item.id} (${item.catalogId})`,
      );
      break;
    }
  }
