...
import type { ApiError, SavedStateBlob, SavedStateMeta, SavedStateRecord, SavedStateSharesResponse, SavedStateSlice } from './types/saved-state.types';
...
export const savedStatesApi = {
  ...
  get: (id: string): Promise<SavedStateRecord> => request(`/${encodeURIComponent(id)}`),
  meta: (id: string): Promise<SavedStateMeta> => request(`/${encodeURIComponent(id)}/meta`),
  create: ({ name, state }: { name: string; state: SavedStateBlob }) => request('/', { method: 'POST', body: { name, state } }),
  updateSlice: (id: string, { name, slice, state }: { name?: string; slice?: Partial<SavedStateSlice> & { catalogId?: string; year?: string | null }; state?: SavedStateBlob }) =>
    request(`/${encodeURIComponent(id)}`, {
      method: 'PUT',
      body: { name, slice, state },
    }),
  remove: (id: string) => request(`/${encodeURIComponent(id)}`, { method: 'DELETE' }),
  listShares: (id: string): Promise<SavedStateSharesResponse> => request(`/${encodeURIComponent(id)}/shares`),
  ...
  requestAccess: (id: string, { requestedRole = 'view', message = '' }: { requestedRole?: string; message?: string } = {}) =>
    request(`/${encodeURIComponent(id)}/request-access`, {
      method: 'POST',
      body: { requestedRole, message },
    }),
  resolveRequest: (id: string, requestId: string, { action, role }: { action?: string; role?: string } = {}) =>
...
export function getSliceFor(savedState: SavedStateBlob | null | undefined, catalogId: string | null, year: string | null): SavedStateSlice {
