...
export const SESSION_ID_PATTERN = /^(?:c[a-z0-9]{20,30}|[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/i;
...
export function parseUrl() {
  ...
  let sessionId = null;
  if (segments.length && SESSION_ID_PATTERN.test(segments[segments.length - 1])) {
    sessionId = segments.pop() || null;
  }
  return { subject: segments[0] || null, year: segments.length > 1 ? segments[1] : null, sessionId };
}

export function updateUrl(subject: string | null, year: string | null, sessionId: string | null) {
  let path = '/';
  if (subject) {
    path += subject;
    if (year) path += `/${year}`;
  }
  if (sessionId) path += `${path.endsWith('/') ? '' : '/'}${encodeURIComponent(sessionId)}`;
...
export function publishAppShell(appState: AppState, refresh: () => void) {
  ...
  document.body.classList.toggle('is-readonly', Boolean(appState.loadedSetting?.role === 'view'));
  refresh();
}

export function showFeedback(appState: AppState, refresh: () => void, message: string, isError = false, actions: FeedbackAction[] = []) {
  appState.feedbackMarkup = isError ? `<span data-state="error">${message}</span>` : message;
...
export function firstYearInSaved(setting: SavedStateRecord | null, catalogId: string | null) {
...
export function getRestoredSettingState(appState: AppState, setting: SavedStateRecord | null, { catalogId, year }: { catalogId?: string | null; year?: string | null } = {}): RestoredState | null {
...
export function canPreviewSetting(appState: AppState, setting: SavedStateRecord) {
...
export function buildViewModel(appState: AppState) {
  const currentCatalog = appState.catalogs.find((catalog) => catalog.id === appState.currentCatalogId) || null;
  const loadedSetting = appState.loadedSetting;
  const effectiveName = appState.settingsName.trim() || loadedSetting?.name || '';
  ...
  const canShare = Boolean(loadedSetting && (loadedSetting.role === 'owner' || loadedSetting.role === 'admin'));
  const canDelete = Boolean(loadedSetting && (loadedSetting.role === 'owner' || loadedSetting.role === 'admin'));

  return {
    currentTitle: currentCatalog?.name || 'Modules',
    theme: appState.theme,
    searchQuery: appState.searchQuery,
    subjects: appState.catalogs.map((catalog) => ({ id: catalog.id, name: catalog.name })),
    ...
    savedPlans: appState.settingsCache.map((setting) => {
      ...
      return { id: String(setting.id), name: setting.name, label: `${setting.name} — ${counts.catalogs} subject${counts.catalogs === 1 ? '' : 's'}, ${counts.selected} selected (${who}${roleTag})` };
    }),
    ...
    viewOnlyBadge: viewOnly ? `View only — shared by ${loadedSetting?.owner?.email || 'the owner'}` : '',
