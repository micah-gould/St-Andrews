...
export function SignupPage() {
  ...
  return (
    <AuthLayout title="Create your account" subtitle="Plan your modules in seconds">
      <form
        className="auth-form"
        noValidate
        onSubmit={async (event) => {
          ...
          try {
            ...
            setMessage({ type: 'error', text: error.message || 'Could not create account.' });
            setSubmitting(false);
          }
        }}
      >
        ...
        <div className="field">
          ...
          <input id="signup-email" name="email" type="email" autoComplete="email" required value={email} onChange={(event) => setEmail(event.target.value)} />
        </div>
        <PasswordField id="signup-password" name="password" label="Password" autoComplete="new-password" minLength={8} value={password} onChange={setPassword} />
        <div className="row">
          <label>
