    ...
    <div className="request-access" id="request-access-overlay">
      <div className="request-access__card">
        ...
        {loadError === "Please sign in to request access." ? (
          <div className="request-access__summary">
            Please <a href="/login">sign in</a> to request access.
          </div>
        ) : null}
        {loadError && loadError !== "Please sign in to request access." ? (
          <div className="request-access__summary">{loadError}</div>
        ) : null}
        {meta ? (
          <>
            <div className="request-access__summary">
              You don't have access to <strong>{meta.name}</strong>, shared by{" "}
              <strong>{meta.owner?.email || "the owner"}</strong>. You can
            ...
            <form
              ...
              onSubmit={async (event) => {
                ...
                setStatus("Sending...");
                try {
                  await savedStatesApi.requestAccess(stateId, {
                    requestedRole,
                    message,
                  });
                  setStatus(
                    `Request sent. You'll get access once ${meta.owner?.email || "the owner"} approves it.`,
                  );
                } catch (err) {
                  setSubmitting(false);
            ...
            >
              ...
              <div className="request-access__actions">
                ...
                <a href="/" className="request-access__home">
                  Go to my plans
                </a>
              </div>
            </form>
        ...
        <p className="request-access__status" role="status" aria-live="polite">
          {status}
        </p>
      </div>
    </div>
...
export async function showRequestAccessOverlay(
  stateId: string,
  { savedStatesApi }: { savedStatesApi: typeof defaultSavedStatesApi },
) {
  ...
  createRoot(container).render(
    <RequestAccessOverlay stateId={stateId} savedStatesApi={savedStatesApi} />,
  );
}
